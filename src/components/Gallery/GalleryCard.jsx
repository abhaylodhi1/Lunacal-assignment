...
export default function GalleryCard({ image, onRemove }) {
  return (
    <div className="min-w-[160px] max-w-[220px] bg-[#0f1113] rounded-xl overflow-hidden shadow-sm relative hover:-translate-y-1 hover:shadow-lg transition-transform duration-200">
      ...
      <div className="p-3 flex items-center justify-between bg-gradient-to-t from-black/60 to-transparent absolute bottom-0 left-0 right-0">
        ...
        <button
          type="button"
          onClick={onRemove}
          className="p-1 rounded bg-black/40  transition"
          title="Remove image"
        >
          <FiTrash2 />
        </button>
